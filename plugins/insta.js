...
const LOAD_ING = "```Wait akk nokkatte...```"
...
Asena.addCommand({ pattern: 'Insta ?(.*)', fromMe: false, desc: "Downloads from instagaram", dontAddCommandList: true }, async (message, match) => { 
    ...
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "```Made By RosyMwol```", quoted: message.data })
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "```Made By RosyMwol```", quoted: message.data })
});
